...
__IO uint16_t ADC3ConvertedValue=0;
 ...
 volatile double  setPoint=40;  //NOTE: if declare as float, when +0.5, the compile will give warning:
...
__IO uint16_t TIM3_CCR1_Val=200, TIM3_CCR2_Val=0,TIM3_CCR3_Val=400,TIM3_CCR4_Val=500;

__IO uint16_t Tim4_CCR; 

void TIM3_PWM_Config(void);
void ExtBtn1_Config(void)     // for GPIO C pin 1 configuration
{
...
void ExtBtn2_Config(void){  //PD2 external button configuration
   GPIO_InitTypeDef   GPIO_InitStructure;
...
void TIM3_PWM_Config(void) //configuring TIM3 for PWMOutput, to conrol the fan speed
{
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef * htim){  //this is for TIM3_pwm callback
...
void  TIM4_Config(void) //configuring the CCR timer for everything else
{
...
void  TIM4_OC_Config(void) //configuring the OC 
{
...
void ADCInit(){ //initializing the ADC to collect the data from the thermometer
...
int main(void){
     ...
     */
	ADCInit();				//running all the configurations
	TIM3_PWM_Config();
	HAL_Init();
  ExtBtn1_Config();
	ExtBtn2_Config();
  ...
  BSP_LED_Init(LED4);
		...
		BSP_PB_Init(GPIO_PIN_1,BUTTON_MODE_EXTI);																
		...
		LCD_DisplayString(3, 2, (uint8_t *) " Lab4 ");
		...
		measuredTemp = 1.0*HAL_ADC_GetValue(&AdcHandle);
		setPoint = measuredTemp * (3000.0/(4095.0*10.0*3.0))+4; //get the initial mesured temprature and add
		// 4 to it, so that it is above the room tempruature.
		
		LCD_DisplayFloat(10, 10, setPoint, 2); //display the intitial setpoint.
...
{														
			if ((*htim).Instance==TIM4) {  //detect the TIM CCR
				OC_Count=OC_Count+1; //increment the OC Count
				
				if (measuredTemp > setPoint){ // if the measured temp is greater that the setpoint, turn the fan on
						level = (measuredTemp-setPoint)*100; //level increases past 50% speed
						if (level>500){level=500;} //if the level goes past 500 (50%), make it 500
						TIM3_CCR2_Val=500+level; // initial speed is 500 and add the level amount
						HAL_TIM_PWM_Start(htim,TIM_CHANNEL_2); //change the speed
						__HAL_TIM_SET_COMPARE(&Tim3_Handle, TIM_CHANNEL_2,TIM3_CCR2_Val);
					} else {
						TIM3_CCR2_Val=0; //if the temp is below setpoint, turn fan off
						HAL_TIM_PWM_Start(htim,TIM_CHANNEL_2);
						__HAL_TIM_SET_COMPARE(&Tim3_Handle, TIM_CHANNEL_2,TIM3_CCR2_Val);
					}
					
				
				if (OC_Count%500==0){ // every 500ms, poll the temprature reading and update the temprature
					HAL_ADC_PollForConversion(&AdcHandle,100);
					measuredTemp = 1.0*HAL_ADC_GetValue(&AdcHandle);
					measuredTemp = measuredTemp * (3000.0/(4095.0*10.0*3.0));
					LCD_DisplayFloat(9,10,measuredTemp,2);
					
				}
				if (OC_Count%10==0){ //every 10 ms, poll the buttons and see if they are pressed
